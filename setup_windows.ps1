...
function Install-PythonIfMissing {
    ...
    if ($pyVer -and $pyVer -ge [version]$PythonReq) {
        Write-Log "✅ Python $pyVer found" Green
        return $true
    }

    Write-Log "⚠️  Python $PythonReq+ required (found: $($pyVer?.ToString() ?? 'none'))" Yellow
    Write-Log "📦 Attempting to install via winget..." Yellow

    if (-not (Test-Command winget)) {
        Write-Log "❌ winget not available. Install Python manually from:" Red
    ...
    try {
        ...
        $pyVer = Get-PythonVersion
        if ($pyVer -and $pyVer -ge [version]$PythonReq) {
            Write-Log "✅ Python $pyVer installed successfully!" Green
            return $true
        }
    } catch {
        Write-Log "❌ Failed to install Python via winget: $_" Red
    }

    Write-Log "⚠️  Could not auto-install Python. Install manually from:" Yellow
...
if (-not $hasPython) {
    Write-Log "❌ Cannot continue without Python." Red
...
try {
    # Download individual files from raw GitHub
    $files = @(
...
try {
    ...
    Write-Log "✅ Pillow installed" Green
} catch {
    Write-Log "⚠️  Could not install Pillow, icon may be basic" Yellow
}
...
try {
    ...
    Pop-Location
    Write-Log "✅ Icon generated" Green
} catch {
    Write-Log "⚠️  Icon generation failed: $_" Yellow
}
...
try {
    ...
    if (Test-Path $IconIco) {
        ...
        # Fallback: use python.exe's icon
    ...
    # Also create a URL file that's more portable
    $urlFile = "$DesktopPath\N64 Emu Setup.url"
@"
[InternetShortcut]
URL=file:///$($AppScript.Replace('\', '/'))
IconIndex=0
IconFile=$IconIco
"@ | Out-File -FilePath $urlFile -Encoding ascii
    
    Write-Log "✅ Desktop shortcut created: $ShortcutPath" Green
} catch {
    Write-Log "⚠️  Could not create desktop shortcut: $_" Yellow
}
...
if (Test-Path $userRom) {
    ...
    Write-Log "   ✅ Copied to ROMs folder" Green
}
...
try {
    ...
    Write-Log "✅ App launched!" Green
} catch {
    Write-Log "⚠️  Could not launch app. Run manually:" Yellow
